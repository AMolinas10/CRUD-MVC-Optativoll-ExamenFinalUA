         ...
         boolean bandera=false;
          if(frmE.txtNss.getText().length()!=11){
           ...
           if(!fechavalida(frmE.txtFechaIngreso.getText())||!fechavalida(frmE.txtFechaNacimiento.getText())){
               JOptionPane.showMessageDialog(null, "el formato de fecha no es valido el formato es Año-Mes-Dia ");
              return; 
           }
            if(!validarEntrada(frmE.txtTelefono.getText())||(frmE.txtTelefono.getText().length()>12||frmE.txtTelefono.getText().length()<8)){
                bandera=true;
                JOptionPane.showMessageDialog(null,"el numero telefónico no puede tener letras y debe ser de longitud válida de 12 digitos");
            }
